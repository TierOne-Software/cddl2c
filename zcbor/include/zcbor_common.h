...
typedef struct {
	uint8_t const *payload; /**< The current place in the payload. Will be
	                             ...
	                             processed. Const because decoding never
	                             writes; encoding casts constness away at
	                             its few write sites. */
...
struct {
	...
	size_t map_elems_processed; /**< The number of elements of an unordered map
	                                 that have been processed. Kept in both
	                                 search modes so exhausted maps fail
	                                 searches in O(1). */
...
/** Bounded string length (like POSIX strnlen, which C11 does not provide;
 *  defining the reserved name ourselves would risk clashing with the libc).
 */
size_t zcbor_strnlen(const char *s, size_t maxlen);

/* Note: there is deliberately no helper for casting functions to
 * zcbor_decoder_t/zcbor_encoder_t. Calling a function through a mismatched
 * function pointer type is undefined behavior (C11 6.3.2.3); write functions
 * with the exact `bool f(zcbor_state_t *, void *)` signature instead and
 * cast the void pointer inside. */
