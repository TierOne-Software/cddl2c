 ...
 * for parameter docs.
 */
/** Like the map end functions, but tolerates (skips) unprocessed elements.
 *
 *  For maps that are extensible: unknown keys added by future schema
 *  versions are skipped instead of causing ZCBOR_ERR_ELEMS_NOT_PROCESSED.
 *  Works after both ordered and unordered map decoding.
 */
...
bool zcbor_search_key_uint(zcbor_state_t *state, uint64_t key); /* integer keys */
